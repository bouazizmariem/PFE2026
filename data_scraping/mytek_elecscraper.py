...
def url_to_name(url: str) -> str:
    ...
    # Sous-catégorie = avant-dernier segment : /electromenager/lavage/machine-a-laver.html
    if len(parts) >= 2:
...
class MytekElecSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._id_to_subcat = {}   # {str(product_id): subcategory_name}
    ...
    def parse(self, response):
        ...
        if subcat_links:
            ...
            # Fallback : traiter la page comme listing direct
            yield from self.parse_listing(response)

    # ── 2. Page listing → extraire IDs depuis INITIAL_PRODUCTS_DATA ──

    def parse_listing(self, response):
        subcategory_name = url_to_name(response.url.split("?")[0])

        # FIX : extraire INITIAL_PRODUCTS_DATA du script JS
        ids_found = 0
        ...
        # Pagination : chercher ?p=N dans les scripts JS aussi
        # Mytek charge la pagination en JS — on incrémente jusqu'à page vide
        current_page = int(re.search(r"[?&]p=(\d+)", response.url).group(1)) \
            if re.search(r"[?&]p=(\d+)", response.url) else 1

        # S'il y avait des produits sur cette page, tenter la page suivante
        if ids_found > 0:
    ...
    def handle_pagination_error(self, failure):
        # Pagination terminée — pas d'erreur à remonter
        pass
...
def run(output_file="data_raw/mytek_Electroproducts.json"):
    ...
    class CollectorSpider(MytekElecSpider):
        def closed(self, reason):
            super().closed(reason)
            # Copier dans la variable externe
    ...
    process.start()   # bloquant jusqu'à la fin du spider
